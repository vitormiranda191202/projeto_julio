...
class FichaApp:
    def __init__(self, root):
        ...
        self.items = []  # itens temporários antes de salvar ficha
    ...
    def create_estoque_widgets(self):
        ...
        ttk.Button(form_frame, text="📊 Relatório", command=self.relatorio_estoque).grid(row=0, column=9, padx=5)
    ...
    def remover_rolos(self):
        ...
        try:
            with SessionLocal() as db:
                ...
                if rolo:
                    # opcional: registrar movimentação de saida manual
                    mov = MovimentacaoEstoque(
                        tipo="saida",
                        produto="Rolo",
                        cor=rolo.cor,
                        ...
                        referencia=f"Remoção manual ID {rolo.id}"
                    )
                    db.add(mov)
    ...
    def relatorio_estoque(self):
        try:
            ...
            if not movs:
                messagebox.showinfo("Aviso", "Nenhuma movimentação encontrada")
                return

            win = tk.Toplevel(self.root)
            win.title("📊 Relatório de Entradas e Saídas")
            win.geometry("1000x600")
            win.configure(bg="#f0f0f0")

            colunas = ("Data", "Tipo", "Produto", "Cor", "Qtd", "Peso", "Referência")
    ...
    def listar_fichas(self):
        try:
                ...
                if messagebox.askyesno("Confirmar", f"Deletar ficha {ficha_id}?"):
                    try:
                        with SessionLocal() as db:
                            ...
                            if ficha_db:
                                # opcional: registrar movimentações de retorno de estoque (não implementado automaticamente)
                                db.delete(ficha_db)
                                db.commit()
            ...
            # Função editar com janela e edição de itens via pop-up
            def editar():
                sel = tree.selection()
                if not sel:
                ...
                # abrir nova sessão para carregar ficha atual
                with SessionLocal() as db:
                ...
                # botão para editar item selecionado (abre pop-up)
                def editar_item():
                ...
                def salvar_edicao():
                    try:
                        ...
                        with SessionLocal() as db:
                            ...
                            # Simplificação: removemos itens antigos e recriamos a partir da tabela.
                            # OBS: não atualizamos estoque/movimentações automaticamente ao editar (pode ser implementado se quiser).
